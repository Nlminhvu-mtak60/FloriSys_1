...
namespace FloriSys._3_BanHang
{
    public partial class ucTraHang : BaseUserControl
    {
        ...
        public ucTraHang()
        {
            ...
            dgvChoTra.SelectionChanged += DgvChoTra_SelectionChanged;
            this.Load += (s, e) => LoadPendingReturns();
        }

        private void DgvChoTra_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvChoTra.CurrentRow != null && dgvChoTra.CurrentRow.Index >= 0)
            {
                string maDon = dgvChoTra.CurrentRow.Cells["MaDon"].Value.ToString();
                SetMaDon(maDon);
            }
        }
