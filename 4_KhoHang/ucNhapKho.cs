...
namespace FloriSys._4_KhoHang
{
    public partial class ucNhapKho : BaseUserControl
    {
        ...
        {
            if (dgvNhap.CurrentRow == null || dgvNhap.CurrentRow.Index < 0)
            { ShowWarning("Vui lòng chọn dòng cần xóa!"); return; }

            int idx = dgvNhap.CurrentRow.Index;
            if (idx < _danhSachNhap.Rows.Count)
            {
                _danhSachNhap.Rows.RemoveAt(idx);
                CapNhatTongCong();
            }
        }
        ...
        private void CapNhatTongCong()
        {
            ...
            foreach (DataRow row in _danhSachNhap.Rows)
            {
                tong += Convert.ToInt32(row["SoLuong"]) * Convert.ToDecimal(row["GiaNhap"]);
            }
            lblTongCong.Text = "Tổng giá trị: " + tong.ToString("N0") + " đ";
        }


        private void cboSanPham_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cboSanPham.SelectedItem is SanPham sp && sp.GiaNhap > 0)
            {
