...
**Chức năng:**
- Tạo phiếu nhập kho → trigger tự động tăng `SoLuongTon` (tự động điền giá nhập khi chọn SP)
...
```sql
Id          INT           PK, IDENTITY
MaDon       NVARCHAR(20)  FK → DON_HANG
TrangThai   NVARCHAR(20)  NOT NULL
ThoiGian    DATETIME      DEFAULT GETDATE()
GhiChu      NVARCHAR(500)
```
**Ý nghĩa:** Bảng lưu trữ lịch sử thay đổi trạng thái của đơn hàng (được trigger `trg_DonHang_Update_Log` tự động thêm vào).
