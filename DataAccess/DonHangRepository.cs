...
namespace FloriSys.DataAccess
{
    ...
    public class DonHangRepository : BaseRepository<DonHang>
    {
        ...
        {
            string sql = @"SELECT dh.MaDon, kh.HoTen AS TenKH, 
                          STRING_AGG(CONCAT(sp.TenSP, ' (x', ct.SoLuong, ')'), ', ') AS TenSP, 
                          SUM(ct.SoLuong) AS SoLuong, 
                          MIN(sp.SoLuongTon) AS SoLuongTon,
                          CASE WHEN MIN(sp.SoLuongTon - ct.SoLuong) >= 0 THEN N'DuHang' ELSE N'KhongDu' END AS TinhTrangKho,
                          dh.HinhThucNhanHang,
                          dh.NgayTao
                          FROM DON_HANG dh
                          INNER JOIN KHACH_HANG kh ON dh.MaKH = kh.MaKH
                          ...
                          GROUP BY dh.MaDon, kh.HoTen, dh.HinhThucNhanHang, dh.NgayTao
